...
import { CalendarList, DateData } from 'react-native-calendars'; // Added
...
const GEOAPIFY_ROUTING_URL = `https://api.geoapify.com/v1/routing`; // <-- Added
...
// ... (mockProperties and mockRegion remain the same) ...
...
// ... (RouteSegmentsList remains the same) ...
// --- RouteSegmentsList (SectionList) ---
const RouteSegmentsList = ({ properties }: { properties: Property[] }) => {
  ...
  return (
    <SectionList
      ...
      ListFooterComponent={<View style={{ height: 40 }} />}  // <- spacer
    />
  );
};

  
// ... (RouteSegmentsGestureList remains the same) ...
  // --- RouteSegmentsGestureList (inside bottom sheet) ---
    ...
    <GestureFlatList
      ...
      renderItem={({ item: [segment, props] }) => (
        <View style={styles.segmentContainer}>
          <View className="" style={styles.segmentHeader}>
            <View className="" style={styles.segmentLine} />
      ...
      style={styles.flatListInSheet}
      showsVerticalScrollIndicator={false}
      ListFooterComponent={<View style={{ height: 50 }} />}  // <- spacer
    />
  );
};


// ... (CustomCheckbox remains the same) ...
...
// ... (RoadTripFilterPanel remains the same) ...
    ...
    if (selectedEndDate) {
        ...
        if (selectedStartDate && selectedStartDate !== selectedEndDate) {
            ...
            while (currentDate < endDate) {
                ...
                markedDates[dateString] = { color: '#F3F4G6', textColor: '#111827' }; // Middle color
                currentDate.setDate(currentDate.getDate() + 1);
            }
    ...
    return (
        <Modal visible={isVisible} animationType="slide" transparent={false} onRequestClose={onClose}>
            <SafeAreaView style={styles.modalContainer}>
                ...
                {/* This View wrapper is the crucial fix for the layout */}
                <View style={{ flex: 1 }}> 
                    <CalendarList
                ...
                {/* Use the specific footer style for Date Picker */}
...
// --- END: Added DatePickerModal ---


// --- START: Added GuestPickerModal ---
...
export default function RoutePlannerPage() {
  ...
  // Store coords, though they aren't used for route fetching *yet*
  const [fromCoords, setFromCoords] = useState<{ lat: number, lon: number } | null>({ lat: 19.0760, lon: 72.8777 });
  const [toCoords, setToCoords] = useState<{ lat: number, lon: number } | null>({ lat: 18.5204, lon: 73.8567 });
  ...
  // --- Added: Map ref & route state ---
  const mapRef = useRef<MapView | null>(null);
  const [routeCoords, setRouteCoords] = useState<{ latitude: number; longitude: number }[] | null>(null);
  const [routeInfo, setRouteInfo] = useState<{ distanceKm: number; timeMin: number } | null>(null);
  const [routingBusy, setRoutingBusy] = useState(false);

  // ... (clampHeightsFromLayout, onContentLayout, gesture, animatedBottomSheetStyle, useEffect remain the same) ...
  ...
  const fetchRoute = async (from: { lat: number; lon: number }, to: { lat: number; lon: number }) => {
    ...
    try {
      ...
      const mode = 'drive'; // 'drive', 'walk', 'bicycle', 'transit'
      const url = `${GEOAPIFY_ROUTING_URL}?waypoints=${encodeURIComponent(waypoints)}&mode=${mode}&apiKey=${GEOAPIFY_API_KEY}`;
      ...
      if (!feature) {
        setRouteCoords(null);
        setRouteInfo(null);
        Alert.alert("No Route", "Could not calculate a route for these points.");
        return;
      }
      ...
      const latlng = lonlatPairs.map(([lon, lat]) => ({ latitude: lat, longitude: lon }));
      setRouteCoords(latlng);

      const distanceMeters = props?.distance || props?.total_distance || 0;
      const timeSeconds = props?.time || props?.total_time || 0;
      setRouteInfo({
  ...
  const handleLocationSearch = async (type: 'from' | 'to') => {
    ...
    try {
        let biasParam = `&bias=proximity:${mockRegion.longitude},${mockRegion.latitude}`;
        ...
        if (data.features && data.features.length > 0) {
            ...
            const displayName = firstResult.properties.formatted || query;
            
            if (type === 'from') {
                setFromCoords({ lat, lon });
                setFromLocationInput(displayName);
            } else {
                setToCoords({ lat, lon });
                setToLocationInput(displayName);
            }

            // Optional immediate refetch if both ends are known
            const haveOtherEnd = type === 'from' ? !!toCoords : !!fromCoords;
            if (haveOtherEnd) {
              const f = type === 'from' ? { lat, lon } : (fromCoords as { lat: number; lon: number });
              const t = type === 'to' ? { lat, lon } : (toCoords as { lat: number; lon: number });
              fetchRoute(f, t);
            }
        } else {
  ...
  return (
      <ThemedView style={[styles.container,  { paddingTop: insets.top}]}>
        ...
        {/* Added TouchableWithoutFeedback to dismiss keyboard */}
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
            <View style={styles.flex1}>
            {/* --- START: Modified Inputs Container --- */}
            ...
            <View style={styles.filterBar}>
                ...
                <Text style={styles.filterResultText}>{filteredProperties.length} stays along {fromLocationInput} → {toLocationInput}</Text>
            </View>
            ...
            <View style={styles.contentArea} onLayout={onContentLayout}>
                ...
                <>
                    ...
                    {routeInfo && (
                      <View style={{
                        ...
                        <Text style={{ fontSize: 12, color: '#111827', fontWeight: '600' }}>
                          {routingBusy ? 'Calculating route...' :
                            `${routeInfo.distanceKm.toFixed(1)} km • ${Math.round(routeInfo.timeMin)} min`}
                        </Text>
                      </View>
                    )}
                    ...
                    >
                      ...
                      {fromCoords && (
                        <Marker
                          coordinate={{ latitude: fromCoords.lat, longitude: fromCoords.lon }}
                          title="From"
                          pinColor="#16A34A"
                        />
                      )}
                      {toCoords && (
                        <Marker
                          coordinate={{ latitude: toCoords.lat, longitude: toCoords.lon }}
                          title="To"
                          pinColor="#DC2626"
                        />
                      )}
...
const styles = StyleSheet.create({
  // ... (All existing styles from route.tsx)
  flex1: { flex: 1 },
  ...
  inputValue: { fontSize: 16, color: '#111827', marginLeft: 8, fontWeight: '500' },
  inputPlaceholder: { fontSize: 16, marginLeft: 12, color: '#6B7280' },
  ...
  segmentHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, marginTop: 8 }, // Added margin top
  segmentLine: { flex: 1, height: 1, backgroundColor: '#E5E7EB' },
  segmentTitle: {
    fontSize: 13, fontWeight: '600', color: '#111827',
    paddingHorizontal: 12, backgroundColor: '#F3F4F6', paddingVertical: 4, borderRadius: 99,
  },
  mapToggleButton: {
  ...
  card: {
    ...
    height: 200, flexDirection: 'row', borderWidth: 1, borderColor: '#E5E7EB',
  },
  ...
  bottomSheet: { // Keep previous dynamic height/bottom logic
  ...
  modalCloseButton: { position: 'absolute', left: 16, top: 16, padding: 4 }, // Changed to 'left' for filters modal
  modalScroll: { flex: 1 },
  ...
  modalFooter: { // This is for the Filter modal
    position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row',
    padding: 16, paddingBottom: 24, backgroundColor: 'white',
  ...
  // --- START: Added Styles ---
  textInputStyle: {
    flex: 1, fontSize: 16, color: '#111827', marginLeft: 8, fontWeight: '500',
  },
  inputText: { 
    flex: 1, fontSize: 16, marginLeft: 12, color: '#111827' 
  },
  inputPlaceholderText: { 
    color: '#6B7280',
    fontSize: 16, marginLeft: 12,
  },
  datePickerModalFooter: { // Specific footer for DatePickerModal to work with flex layout
  ...
  guestModalContent: { 
    ...
    width: '85%', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, 
    shadowOpacity: 0.25, shadowRadius: 4, elevation: 5, 
  },
  guestModalTitle: { 
    fontSize: 18, fontWeight: '600', marginBottom: 20, textAlign: 'center', 
  },
  guestControlRow: { 
  ...
  guestButton: { 
    ...
    borderColor: '#D1D5DB', justifyContent: 'center', alignItems: 'center', marginHorizontal: 8, 
  },
  disabledGuestButton: { 
    borderColor: '#E5E7EB', 
  },
  guestCount: { 
    fontSize: 18, fontWeight: '600', minWidth: 30, textAlign: 'center', 
  },
  confirmGuestButton: { 
    backgroundColor: '#111827', paddingVertical: 14, 
  ...
  map: {
    flex: 1, // Allows the map to fill available space flexibly
  },
  // --- END: Added Styles ---
});
